...
def evolve_to_3tev(m_u_MZ, m_c_MZ, m_t_MZ, mt_msbar, alphas_mz):
    ...
    Path
    ----
    M_Z ──[nf=5, UP]──▶ m_t(m_t) ──[match 5→6]──▶ 3 TeV [nf=6, UP]

    Uses the EXISTING qcd.py running and matching functions — no
    reimplementation of the QCD machinery.
    ...
    # ── M_Z → top threshold (nf=5, running UP) ──
    a5_at_thresh_4pi = run_alpha_s(a_mz_4pi, MZ, mu_thresh, 5)

    m_u_5 = _run_mass_segment(MZ, m_u_MZ, a_mz_4pi, mu_thresh, 5)
    m_c_5 = _run_mass_segment(MZ, m_c_MZ, a_mz_4pi, mu_thresh, 5)
    m_t_5 = _run_mass_segment(MZ, m_t_MZ, a_mz_4pi, mu_thresh, 5)
    ...
    # ── Threshold → 3 TeV (nf=6, running UP) ──
    a6_at_3tev_4pi = run_alpha_s(a6_4pi, mu_thresh, MU_HIGH, 6)

    m_u_3tev = _run_mass_segment(mu_thresh, m_u_6, a6_4pi, MU_HIGH, 6)
    m_c_3tev = _run_mass_segment(mu_thresh, m_c_6, a6_4pi, MU_HIGH, 6)
    m_t_3tev = _run_mass_segment(mu_thresh, m_t_6, a6_4pi, MU_HIGH, 6)
